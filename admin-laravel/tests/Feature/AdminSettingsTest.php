...
class AdminSettingsTest extends TestCase
{
    ...
    public function test_there_is_no_vector_store_choice_to_get_wrong(): void
    {
        // The engine uses the store that matches the database it is connected
        // to. A choice that disagreed with that database could only break
        // search, and its default did, on every SQLite install.
        $this->actingAs($this->superAdmin())
            ->get(route('admin.settings'))
            ->assertOk()
            ->assertDontSee('name="vector_driver"', false)
            ->assertDontSee('Vector store');

        $this->assertArrayNotHasKey('vector_driver', AppSetting::DEFAULTS);
    }
