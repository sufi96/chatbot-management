...
class RetrievalPlaygroundTest extends TestCase
{
    ...
    public function test_the_similarity_floor_is_sent_to_the_engine(): void
    {
        Http::fake(['*' => Http::response(['results' => []], 200)]);
        ...
        Http::assertSent(fn ($request) => $request['min_similarity'] === 0.65);
    }

    public function test_each_passage_shows_how_similar_it_is(): void
    {
        Http::fake(['*' => Http::response(['results' => [
            ['chunk_id' => 7, 'source_id' => 'kbs_1', 'content' => 'Thirty days.',
             'score' => 0.0328, 'heading_path' => '', 'similarity' => 0.6912],
        ]], 200)]);
