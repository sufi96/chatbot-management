...
def make_store(session) -> VectorStore:
    """The store for the database this session is actually connected to.

    There used to be a vector_driver setting. A setting about the database can
    only disagree with it, and disagreeing breaks: pgvector's operators do not
    exist on SQLite, and a SQLite blob is not pgvector's type. It defaulted to
    pgvector, so a development install on SQLite, or the engine running on its
    SQLite fallback, failed every search with a syntax error the chat route
    swallowed.
    """
    bind = getattr(session, "bind", None)
    dialect = getattr(getattr(bind, "dialect", None), "name", "")

    return SqliteVectorStore(session) if dialect == "sqlite" else PgVectorStore(session)
