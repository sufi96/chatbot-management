...
@pytest.mark.asyncio
async def test_the_message_being_asked_is_sent_once():
    """The widget sends the current message as the last history turn as well.
    Appended again, the model would read the visitor's question twice."""
    seen, handler = capture(delta(content="Hi"))
    await run_stream(handler, user_message="and the warranty?", history=[
        {"role": "user", "content": "How much is the X200?"},
        {"role": "assistant", "content": "RM 399."},
        {"role": "user", "content": "and the warranty?"},
    ])

    turns = [(m["role"], m["content"]) for m in seen["body"]["messages"] if m["role"] != "system"]
    assert turns == [("user", "How much is the X200?"), ("assistant", "RM 399."),
                     ("user", "and the warranty?")]


@pytest.mark.asyncio
async def test_an_earlier_identical_question_is_kept():
    """Asking the same thing twice is a real conversation, not a duplicate."""
    seen, handler = capture(delta(content="Hi"))
    await run_stream(handler, user_message="hello", history=[
        {"role": "user", "content": "hello"},
        {"role": "assistant", "content": "Hi! How can I help?"},
    ])

    turns = [(m["role"], m["content"]) for m in seen["body"]["messages"] if m["role"] != "system"]
    assert turns == [("user", "hello"), ("assistant", "Hi! How can I help?"), ("user", "hello")]


@pytest.mark.asyncio
async def test_a_history_without_the_message_is_left_as_it_is():
    """An older widget, or another client, may not include the message."""
    seen, handler = capture(delta(content="Hi"))
    await run_stream(handler, user_message="and the warranty?", history=[
        {"role": "user", "content": "How much is the X200?"},
        {"role": "assistant", "content": "RM 399."},
    ...
    turns = [(m["role"], m["content"]) for m in seen["body"]["messages"] if m["role"] != "system"]
    assert turns[-1] == ("user", "and the warranty?")
    assert len(turns) == 3
