...
@pytest.mark.asyncio
async def test_a_sqlite_database_is_left_alone_whatever_the_old_setting_said(session):
    """vector_driver defaulted to pgvector, and on SQLite the ALTER it led to is
    a syntax error. The database decides, as it does for the vector store."""
    from kb.store import SqliteVectorStore
    await session.merge(AppSetting(key="vector_driver", value="pgvector"))
